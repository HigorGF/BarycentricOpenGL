...
void main()
{
    ...
    float ar = iResolution.x / iResolution.y;
        ...
        vec2 uv = (gl_FragCoord.xy / iResolution.xy * 2. - 1.) * vec2(ar, 1.);
            if(iMouse.z==1.0)
             mc = (iMouse.xy    / iResolution.xy * 20. - 5.) * vec2(ar, 1.);
...
*/
        vec3 c1=vec3(6.0,0.4,0.1);
        vec3 c2=vec3(1.0,0.8,0.1);
        vec2 p =gl_FragCoord.xy/iResolution.xy;
        p.x*=iResolution.x/iResolution.y;
        vec3 col = mix(c1,c2,p.y);
        vec2 q =p-vec2(0.1,0.17);
        float r =0.1;
        r+=0.045*cos(atan(q.x,q.y)*1-30.0*q.x +sin(iTime*1.14159));
        r+=0.01*sin(atan(q.x,q.y)*20.0);
        col*=dist_01(q,r);

        vec2 x = p-vec2(0.1, 0.2+0.4*cos(2.0*PI*iTime*0.0125));
        r=0.2;
        ...
        float f= 0.1+smoothstep(0,0.1,0.6-x.y);
